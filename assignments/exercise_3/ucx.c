...
void bench(char *sdata, int iter, int warmup, size_t data_size)
{
    ...
    //     for (int i = 0, offset = 0; i < iter; i++, offset += data_size)
    //     {
    //         ucp_status = ucp_get_nbx(endpoints[0],
    //                                  &sdata[offset],
    //                                  data_size,
    //                                  remote_addresses[0] + offset,
    //                                  rkeys[0],
    //                                  &req_param);
    //         if (UCS_PTR_IS_PTR(ucp_status))
    //         {
    ...
    for (int i = 0, offset = 0; i < iter; i++, offset += data_size)
    {
        ...
        if (my_pe == 0)
        {
            ...
            ucp_worker_wait_mem(ucp_worker, &sdata[offset]);
        }
        else
        {
            ucp_worker_wait_mem(ucp_worker, &sdata[offset]);
            ucp_status = ucp_put_nbx(endpoints[0],
                                     &sdata[offset],
                                     data_size,
                                     remote_addresses[0] + offset,
                                     rkeys[0],
...
int main(void)
{
    ...
    {
        int iter = HUGEPAGE / i > 1024 ? 1024 : HUGEPAGE / i;
        bench(sdata, iter, 10, i);
    }
