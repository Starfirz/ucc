                             ...
                             size_t r_rkey_buf_size)
{
    ...
    total_size += FIELD_SIZEOF(allgather_offload_args_t, r_count);
...
// 这里是打包发给DPU，这里的参数task才决定了buffer的类型
                             ...
                             void *args_buf)
{
    ...
    *((uint64_t *)(args_buf + total_size)) = r_length;
    ...
    //这里和allgather_offload_args_t中的成员应该是对应的，r_start和r_length之后就是r_count
    //r_count代表了接受的数量，所以要用dst中的info
