...
#ifndef BCAST_OFFLOAD_H_
#define BCAST_OFFLOAD_H_
...
enum bcast_ext_am_id {
    ...
    UCC_TL_UCP_BCAST_HOST_ARRIVE_AM_ID = 611,
     ...
     * for the rank.
     */
    UCC_TL_UCP_BCAST_DPU_DONE_AM_ID,

    /* Last AM message ID */
    UCC_TL_UCP_BCAST_LAST_AM_ID
};

enum bcast_offload_subop_type {
    BCAST_OFFLOAD_SEND = 0,
    BCAST_OFFLOAD_RECV = 1
};
 ...
 * information for the DPU to start the bcast operation.
 */
typedef struct bcast_offload_args {
    ...
    // 从root进程接受的数量
    uint64_t  r_count;         /* receive count in bytes */
...
} bcast_offload_subop_t;

/* DPU RTS notification bookkeeping list item. */
typedef struct bcast_offload_dpu_rts_event {
    ucs_list_link_t                  super;      /* notification list item */
    bcast_offload_dpu_rts_t     *payload;    /* payload buffer */
} bcast_offload_dpu_rts_event_t;

/* RDMA READ bookkeeping list item. */
typedef struct bcast_offload_rdma_read {
    ucs_list_link_t              super;      /* op list item */
    ucs_status_t                 status;     /* op status */
    ...
    bcast_offload_coll_t    *op;         /* original receive op */
    bcast_offload_subop_t   *subop;      /* original receive subop */
} bcast_offload_rdma_read_t;
#endif //BCAST_OFFLOAD_H_
