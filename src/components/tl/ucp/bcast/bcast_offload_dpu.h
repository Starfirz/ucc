...
static int
import_memh_b(execution_context_t *context, void *address, size_t length,
...
static int
add_receive_b(execution_context_t *context, bcast_offload_coll_t *op,
            bcast_offload_subop_t *subop,
...
static int
complete_op_b(execution_context_t *context, bcast_offload_coll_t *op)
{
    int rc;
...
static int
host_arrive_am_cb_b(struct dpu_offload_ev_sys *ev_sys,
                  ...
                  size_t hdr_size, void *data, size_t data_len)
{
    ...
    rc = import_memh_b(context, (void *)args->s_start, args->s_length,
                     args->s_rkey_buf, &op->s_memh);
    if (rc) {
        return UCS_ERR_NO_MEMORY;
    }
    rc = import_memh_b(context, (void *)args->r_start, args->r_length,
...
static int
dpu_rts_am_cb_b(struct dpu_offload_ev_sys *ev_sys, execution_context_t *context,
              ...
              size_t data_len)
{
    ...
    rc = add_receive_b(context, op, subop, args);
    if (rc) {
        ucs_error("add_receive_b() failed");
        return UCS_ERR_NO_MESSAGE;
    }
...
static int
dpu_ack_am_cb_b(struct dpu_offload_ev_sys *ev_sys, execution_context_t *context,
              struct am_header *hdr, size_t hdr_size, void *data,
              size_t data_len)
{
    ucs_debug("received %zu bytes from client %zu for message type %zu context %p",
...
static int progress_rts_b(execution_context_t *context)
{
    int rc;
            ...
            if (rts_item->payload->coll_type == op_item->args.coll_type &&
                ...
                ucs_list_for_each_safe(subop_item, subop_tmp, &op->r_pending,
                    ...
                    if (rts_item->payload->rank == subop_item->peer) {
                        ...
                        /* add this receive op to pending_receives list */
                        rc = add_receive_b(context, op, subop, rts->payload);
                        if (rc) {
                            ucs_error("add_receive_b() failed");
                            return UCS_ERR_NO_MESSAGE;
                        }
...
/* progress receives */
static int progress_receive_b(execution_context_t *context)
{
    int rc;
...
static int progress_ops_b(execution_context_t *context)
{
    int rc;
    rc = progress_rts_b(context);
    rc = progress_receive_b(context);
